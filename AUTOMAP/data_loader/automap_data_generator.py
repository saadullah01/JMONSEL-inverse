...
class DataGenerator:
    def __init__(self, config):
        ...
        train_in_dict = loadmat(train_in_file)
        
        print('*** LOADING TRAINING OUTPUT DATA ***')
        train_out_dict = loadmat(train_out_file)

        train_in_key = list(train_in_dict.keys())[3]
        train_out_key = list(train_out_dict.keys())[3]
...
class ValDataGenerator:
    def __init__(self, config):
        ...
        test_in_dict = loadmat(test_in_file)

        print('*** LOADING TESTING OUTPUT DATA ***')
        test_out_dict = loadmat(test_out_file)

        test_in_key = list(test_in_dict.keys())[3]
        test_out_key = list(test_out_dict.keys())[3]
