...
dest = r"C:\Users\vaibh\Documents\503-project\JMONSEL\JMONSEL\Examples\Si_dataset";
...
# tablePath = "C:\Users\mxpeng\Researches\JMONSEL_distribution\JMONSEL\ScatteringTables\glassyCTables"+PathSep
...
layer2Region = monte.addSubRegion(layer1Region,SiMSM,layer2)  
...
deepRegion = monte.addSubRegion(layer3Region,SiMSMDeep,layer4)
...
for coordx in range(-10,10,4):
    for coordy in range(10,-10,-4):
        filename = dest + PathSep + "Si" + str(int(coordx))+"," + str(int(coordy)) + "data.txt"
        file = open(filename, 'w')
        print filename
        line = mon.NShapes.createLine(-h,w,linelength,thetal,thetar,radl,radr)
